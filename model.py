...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        query_output = self.fc_query(query).view(b_size, -1, self.n_heads, self.head_size).permute(0, 2, 1, 3)
        key_output = self.fc_key(key).view(b_size, -1, self.n_heads, self.head_size).permute(0, 2, 1, 3)
        value_output = self.fc_value(value).view(b_size, -1, self.n_heads, self.head_size).permute(0, 2, 1, 3)
