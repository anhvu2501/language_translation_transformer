...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # Calculate Q, K, V
        query_output = self.fc_query(query).view(batch_size, -1, self.n_heads, self.head_size).permute(0, 2, 1, 3)
        key_output = self.fc_query(key).view(batch_size, -1, self.n_heads, self.head_size).permute(0, 2, 1, 3)
        value_output = self.fc_query(value).view(batch_size, -1, self.n_heads, self.head_size).permute(0, 2, 1, 3)
...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_size, n_heads, ff_size, dropout, device):
        ...
        self.self_atten = MultiHeadAttentionLayer(hidden_size=hidden_size, n_heads=n_heads, dropout=dropout, device=device)
    ...
    def forward(self, input, input_mask):
        ...
        output = self.ff_layer_norm(atten_norm + nn.dp(ff_result))
...
class Encoder(nn.Module):
    def __init__(self, input_size, hidden_size, n_layers, n_heads, ff_size, dropout, device, MAX_LENGTH=100):
        ...
        for _ in range(n_layers):
            encoding_layer.append(EncoderLayer(hidden_size, n_heads, ff_size, dropout, device))
        self.encode_sequence = nn.Sequential(*encoding_layer)
...
class Decoder(nn.Module):
    ...
    def forward(self, target, encoded_input, target_mask, input_mask):
        ...
        pos = torch.arange(0, target_size).unsqueeze(0).repeat(batch_size, 1).to(self.device)
